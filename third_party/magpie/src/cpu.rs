...
use magpie_platform::cpu::{CpuResponse, CpuResponseKind};
...
pub fn handle_request() -> nng::Buffer {
    ...
    let response = Response {
        body: Some(ResponseBody::Cpu(CpuResponse {
            response: Some(CpuResponseKind::Cpu(cpu)),
        })),
    };
    ...
    log::debug!(
        "PERF: CPU information loaded and serialized in {:?}",
        start.elapsed()
    );

    cache.refresh_cpu_async();
