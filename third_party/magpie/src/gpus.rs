...
pub fn handle_request() -> nng::Buffer {
    ...
    let response = Response {
        body: Some(ResponseBody::Gpus(GpusResponse {
            response: Some(GpusResponseKind::Gpus(GpuMap { gpus })),
        })),
    };
    let mut buffer = nng::Buffer::new(response.encoded_len());
    response.encode_raw(&mut buffer);

    log::debug!("PERF: GPUs loaded and serialized in {:?}", start.elapsed());

    cache.refresh_gpus_async();
